...
def get_phase_info(task_status: TaskStatus) -> Optional[Dict[str, Any]]:
    ...
    """
    tool_def = next((td for td in TOOL_DEFINITIONS.values() 
                    if task_status in td.entry_statuses), None)
    if tool_def:
        return {
            "status": task_status,
            "tool_name": tool_def.name,
            "description": tool_def.description.strip(),
            "exit_status": tool_def.exit_status
        }
    return None
...
def get_next_status(current_status: TaskStatus) -> Optional[TaskStatus]:
    ...
    """
    tool_def = next((td for td in TOOL_DEFINITIONS.values() 
                    if current_status in td.entry_statuses), None)
