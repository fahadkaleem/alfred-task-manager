...
async def test_provide_review_with_feedback_overrides_approval():
    ...
    with patch('alfred.tools.provide_review_logic.load_task', return_value=mock_task), \
        ...
        mock_turn_manager.request_revision.assert_called_once_with(
            task_id="TEST-001",
            state_to_revise="reviewing",
            feedback=feedback,
            requested_by="human"
        )
