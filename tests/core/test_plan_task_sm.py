...
class TestPlanTaskStateMachine:
    ...
    def test_full_success_flow(self):
        ...
        # DESIGN -> REVIEW_DESIGN -> GENERATE_SLOTS
        tool.submit_design()
        assert tool.state == PlanTaskState.REVIEW_DESIGN.value
        tool.ai_approve()
        assert tool.state == PlanTaskState.GENERATE_SLOTS.value
        
        # GENERATE_SLOTS -> REVIEW_PLAN -> VERIFIED
        tool.submit_generate_slots()
    ...
    def test_revision_cycle_generate_slots(self):
        """Test revision cycle at generate_slots stage."""
        tool = PlanTaskTool(task_id="TS-01")
        
        # Move to GENERATE_SLOTS state
        tool.submit_contextualize()
        tool.ai_approve()
        tool.submit_strategize()
        tool.ai_approve()
        ...
        # Submit generate_slots work
        tool.submit_generate_slots()
        assert tool.state == PlanTaskState.REVIEW_PLAN.value
        
        # Request revision - should go back to GENERATE_SLOTS
        tool.request_revision()
        assert tool.state == PlanTaskState.GENERATE_SLOTS.value