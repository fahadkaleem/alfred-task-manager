    ...
    def test_invalid_task_id(self) -> Dict[str, Any]:
        ...
        for task_id, description in test_cases:
            try:
                ...
                results.append({"task_id": task_id, "description": description, "expected_error": "TaskNotFoundError or ValidationError", "actual_error": str(e), "test_passed": False})

        return {"test_name": "invalid_task_id", "results": results, "total_cases": len(test_cases), "passed_cases": sum(1 for r in results if r.get("test_passed", False))}
    ...
    def test_malformed_artifacts(self) -> Dict[str, Any]:
        ...
        for artifact, description in malformed_artifacts:
            try:
                ...
                results.append({"artifact": artifact, "description": description, "expected_error": "ValidationError or SchemaError", "actual_error": str(e), "test_passed": False})

        return {"test_name": "malformed_artifacts", "results": results, "total_cases": len(malformed_artifacts), "passed_cases": sum(1 for r in results if r.get("test_passed", False))}
    ...
    def test_invalid_state_transitions(self) -> Dict[str, Any]:
        ...
        for from_state, to_state, description in invalid_transitions:
            try:
                ...
                results.append(
                    {"from_state": from_state, "to_state": to_state, "description": description, "expected_error": "InvalidStateTransitionError", "actual_error": str(e), "test_passed": False}
                )

        return {"test_name": "invalid_state_transitions", "results": results, "total_cases": len(invalid_transitions), "passed_cases": sum(1 for r in results if r.get("test_passed", False))}

    def run_all_error_tests(self) -> List[Dict[str, Any]]:
        """Execute all error scenario tests."""
        return [self.test_invalid_task_id(), self.test_malformed_artifacts(), self.test_invalid_state_transitions()]
